...
const TARGET_EMAIL = 'drmistysenz@gmail.com';
const TARGET_NAME = 'Dr. Misty Senz';
...
// Helper to generate the email HTML with both tier options
    ...
    <html>
      ...
      <body style="margin: 0; padding: 0; background-color: #050E1D;">
        <div style="${BRAND_STYLE}">
          <p style="text-transform: uppercase; letter-spacing: 0.2em; font-size: 11px; font-weight: 800; color: #E67E22; margin-bottom: 20px;">Admission Decision: Approved</p>
...
async function sendRescueEmail() {
  console.log(`🚀 Initiating Rescue Send for ${TARGET_NAME}...`);
  try {
    const { data, error } = await resend.emails.send({
      from: FROM_EMAIL,
      to: TARGET_EMAIL,
      subject: 'Admission Decision: Approved (Enrollment Options)',
    ...
    if (error) {
      ...
      console.log("✅ Success! Email delivered to Resend queue. ID:", data.id);
    }
  } catch (err) {

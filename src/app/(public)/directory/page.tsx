...
import { Search, MapPin, Filter, Star, ShieldCheck, ArrowRight, Zap, Globe, MousePointerClick, Heart, Sparkles, X, Target } from "lucide-react";
...
function DirectoryContent({ initialData }: { initialData: { doctors: any[], total: number } }) {
  ...
  // Re-fetch only if region changes
  useEffect(() => {
    if (region.code !== 'US') { 
      fetchDoctors();
    }
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        try {
          ...
          // For now, we'll mock the result for the demo
  ...
  return (
    <div className="min-h-screen bg-neuro-cream">
      ...
      {/* Wizard Overlay */}
      {isWizardOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 backdrop-blur-md bg-neuro-navy/40">
          <SmartMatchWizard 
            onMatch={(criteria) => {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7">
            <div className="bg-slate-200 rounded-[3rem] p-2 shadow-xl border border-gray-100 h-[700px] sticky top-8 overflow-hidden relative group">
               <GlobalNetworkMap key={region.code} />
            </div>
          </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                            <div className="flex items-center gap-1.5 mb-0.5">
                              <h3 className="font-bold text-lg text-neuro-navy group-hover:text-neuro-orange transition-colors">Dr. {doc.last_name}</h3>

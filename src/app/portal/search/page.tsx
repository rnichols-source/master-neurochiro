  ...
  return (
    <DashboardLayout>
      <div className="space-y-10">
        <div>
          <p className="text-brand-orange font-black uppercase tracking-[0.4em] text-[10px] mb-2">Search Results</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 ml-2">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-navy/40">Training Units</h3>
            </div>
            ...
            <div className="space-y-4">
              {modules.length > 0 ? modules.map((mod: any) => (
                <Link key={mod.id} href={`/portal/curriculum/${mod.weeks.slug}/${mod.slug}`} className="block group">
                  <EliteCard className="p-6 hover:border-brand-orange/40 transition-all">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-[8px] font-black uppercase text-brand-orange mb-1">{mod.weeks.title}</p>
                ...
                <div className="p-8 border-2 border-dashed border-brand-navy/5 rounded-[2rem] text-center">
                  <p className="text-[10px] font-bold text-brand-navy/20 uppercase tracking-widest">No matching units</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 ml-2">
              ...
              <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-brand-navy/40">Clinical Resources</h3>
            </div>
              ...
              {resources.length > 0 ? resources.map((res: any) => (
                <a key={res.id} href={res.url} target="_blank" rel="noopener noreferrer" className="block group">
                  <EliteCard className="p-6 hover:border-brand-orange/40 transition-all">
                    <div className="flex justify-between items-center">
                      <div>
                        <p className="text-[8px] font-black uppercase text-brand-navy/40 mb-1">{res.type}</p>
                ...
                <div className="p-8 border-2 border-dashed border-brand-navy/5 rounded-[2rem] text-center">
                  <p className="text-[10px] font-bold text-brand-navy/20 uppercase tracking-widest">No matching resources</p>
                </div>
              )}
            </div>
          </div>

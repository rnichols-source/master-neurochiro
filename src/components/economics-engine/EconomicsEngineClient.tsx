      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-brand-navy/5 pb-6">
        <div>
          <div className="inline-flex items-center gap-2 px-3 py-1 bg-brand-orange/10 rounded-full text-brand-orange mb-4">
            <Calculator size={14} /><p className="text-[10px] font-black uppercase tracking-widest">Structural Pricing Builder</p>
          </div>
          <h1 className="text-4xl md:text-5xl font-black text-brand-navy tracking-tighter leading-none">Economics Engine</h1>
          <p className="text-brand-gray font-medium mt-2 max-w-xl text-lg">Eliminate emotional pricing and replace it with structural clarity.</p>
        </div>
        <div className="flex gap-4 w-full md:w-auto">
          <BrandButton variant="outline" onClick={() => setShowEdu(!showEdu)} className={cn("text-[10px] gap-2 flex-1", showEdu && "bg-brand-navy text-white")}><BookOpen size={14} /> {showEdu ? "Hide Logic" : "Explain Math"}</BrandButton>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <AnimatePresence mode="wait">
            {step === 1 && (
              <motion.div key="s1" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
                <div><h2 className="text-2xl font-black text-brand-navy tracking-tight">Step 1: Monthly Overhead</h2><p className="text-sm text-brand-gray font-medium">List only recurring expenses. Divide annuals by 12.</p></div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {[
                    ...
                    { id: 'insurance', label: 'Insurances' }, { id: 'marketing', label: 'Marketing' },
                    { id: 'equipment', label: 'Equipment Payments' }, { id: 'professional', label: 'CPA / Legal' },
                    { id: 'loans', label: 'Loan Payments' }, { id: 'misc', label: 'Misc Recurring' },
                  ].map((f) => (
            ...
            {step === 2 && (
              <motion.div key="s2" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                <div><h2 className="text-2xl font-black text-brand-navy tracking-tight">Step 2: Compensation & Buffer</h2><p className="text-sm text-brand-gray font-medium">Define your net income goal and safety reserves.</p></div>
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-sm font-black text-brand-navy uppercase">Target Monthly Net Owner Pay</label>
                    <div className="relative">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-brand-navy/40 ml-1 text-center block">Safety / Reinvestment Buffer</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-brand-navy/40 ml-1 text-center block">Tax Allocation (%)</label>
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                <div><h2 className="text-2xl font-black text-brand-navy tracking-tight">Step 3: Realistic Capacity</h2><p className="text-sm text-brand-gray font-medium">Define your maximum and actual current visit volume.</p></div>
                <div className="grid grid-cols-1 gap-6">
                  <div className="space-y-2">
                    <label className="text-sm font-black text-brand-navy uppercase block">Maximum Weekly Visit Capacity</label>
                    ...
                    <p className="text-[10px] text-brand-gray font-bold uppercase tracking-widest mt-2 ml-2">Monthly Capacity: {monthlyCap} Visits</p>
                  </div>
                  <div className="grid grid-cols-2 gap-6">
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-brand-navy/40 ml-1">Current Weekly Visits</label>
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-widest text-brand-navy/40 ml-1">Current CPV</label>
            ...
            {step === 4 && (
              <motion.div key="s4" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                <div><h2 className="text-2xl font-black text-brand-navy tracking-tight text-center">Structural Pricing Roadmap</h2><p className="text-sm text-brand-gray font-medium text-center">Your tiered path to reaching required economics.</p></div>
                  ...
                  {[
                    ...
                    <EliteCard key={i} className="p-6 border-brand-navy/5 bg-white relative overflow-hidden group">
                      <div className="flex justify-between items-center relative z-10">
                        ...
                        <div className="text-right"><p className="text-[9px] font-black text-brand-navy/30 uppercase mb-1">Target CPV</p><p className="text-3xl font-black text-brand-navy tracking-tighter">${s.val}</p></div>
                      </div>
              ...
              <motion.div key="s5" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                ...
                <div className="grid grid-cols-1 gap-4">
                  {[
                    ...
                    { label: "CP2 - Reconstruction (36 Visits)", val: cp2, desc: "Neural remodeling focus." },
                    { label: "CP3 - Mastery (48 Visits)", val: cp3, desc: "Severe dysfunction reconstruction." },
                  ].map((p, i) => (
            ...
            {step === 6 && (
              <motion.div key="s6" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
                <div><h2 className="text-2xl font-black text-brand-navy tracking-tight text-center">Future Forecast</h2><p className="text-sm text-brand-gray font-medium text-center">Strategic scenarios for your clinic's financial future.</p></div>
                <div className="grid grid-cols-1 gap-4">
                  {[
          ...
          {showEdu && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="p-8 bg-brand-navy text-white rounded-[2.5rem] space-y-4 relative overflow-hidden">
              ...
              <div className="flex gap-3 items-center text-brand-orange"><Info size={18} /><p className="text-xs font-black uppercase tracking-widest">Clinical Explanation Mode</p></div>
              <div className="space-y-4 text-sm text-white/70 leading-relaxed font-medium">
                <p><strong>CPV vs Volume:</strong> Fixing CPV is the primary stabilizer. Increasing volume without fixing CPV leads to burnout.</p>
                <p><strong>Stability Buffer:</strong> Stable clinics architect a reserve ($3k-$6k) to handle upgrades and emergencies.</p>
                <p><strong>Emotional Pricing:</strong> Math removes guilt and replaces it with biological necessity.</p>
              </div>
            </motion.div>
          )}
        </div>
        ...
        <div className="lg:col-span-5 space-y-6 sticky top-24">
          <EliteCard className="p-8 bg-brand-navy text-white border-none shadow-2xl relative overflow-hidden">
            ...
            <div className="space-y-10 relative z-10">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-[10px] font-black uppercase tracking-widest text-white/40"><span>Schedule Utilization</span><span className={cn(util > 80 ? "text-brand-orange" : "text-green-400")}>{util}%</span></div>
              ...
              <div className="grid grid-cols-2 gap-6 pt-6 border-t border-white/10">
                ...
                <div><p className="text-[9px] font-black uppercase text-white/40 mb-1">Monthly Gap</p><p className={cn("text-2xl font-black", mGap > 0 ? "text-red-400" : "text-green-400")}>{mGap > 0 ? `-$${mGap.toLocaleString()}` : "Balanced"}</p></div>
                <div><p className="text-[9px] font-black uppercase text-white/40 mb-1">Annual Opp</p><p className="text-2xl font-black text-brand-orange">${aGap.toLocaleString()}</p></div>
              </div>
              {fullCapRev < reqRev && (
                <div className="p-5 bg-red-500/20 border border-red-500/40 rounded-3xl flex gap-4 items-start"><AlertCircle className="text-red-500 w-6 h-6 shrink-0" /><div><p className="text-xs font-black uppercase tracking-widest text-red-400 mb-1">Structural Failure</p><p className="text-[10px] text-white/60 font-medium leading-relaxed italic">Even if your schedule was 100% full at your current pricing, you would only produce ${Math.round(fullCapRev).toLocaleString()}/mo. Volume is NOT your problem. Your architecture is mathematically incapable of supporting your goals.</p></div></div>
              )}
            </div>
          </EliteCard>
          <div className="bg-white border border-brand-navy/5 rounded-[2.5rem] p-8 space-y-6 shadow-sm">
            <p className="text-[10px] font-black uppercase tracking-widest text-brand-navy/40 text-center">Monthly Distribution Breakdown</p>
            <div className="space-y-4">
              {[ { label: "Overhead", val: overhead }, { label: "Net Owner", val: data.desiredIncome }, { label: "Tax (30%)", val: tax }, { label: "Buffer", val: data.buffer } ].map((item, i) => (
